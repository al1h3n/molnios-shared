...
alias lt="eza --icons always -TL 2"
...
_radio_extract_id() {
  ...
  id=$(print -r -- "$input" \
    | grep -oP '(?:youtu\.be/|youtube\.com/(?:watch\?v=|live/|shorts/)|[?&]v=)\K[A-Za-z0-9_-]{11}' \
    | head -n1)

  if [[ -z $id && $input =~ ^[A-Za-z0-9_-]{11}$ ]]; then
    id=$input
  fi
...
radio() {
  ...
  _radio_deps_ok mpv gum nc || return 1

  local sock="${XDG_RUNTIME_DIR:-/tmp}/mpv-radio.sock"
  local example=dQw4w9WgXcQ
  local video=E2vONfzoyRI volume=50

  if [[ -n $1 ]]; then
    local id
    id=$(_radio_extract_id "$1")
    if [[ -z $id ]]; then
      gum style --foreground 1 "radio: couldn't extract a YouTube ID from '$1'"
      return 1
    fi
    video=$id
  fi
  ...
  if [[ -n $2 ]]; then
    if [[ $2 != <-> || $2 -lt 0 || $2 -gt 100 ]]; then
      gum style --foreground 1 "radio: volume must be an integer 0-100, got '$2'"
      return 1
    fi
    volume=$2
  fi
  ...
  _radio_mpv_alive() {
    [[ -S $sock ]] && print -r -- '{"command":["get_property","pid"]}' \
  ...
  _radio_start() {
    ...
    mpv --msg-level=all=error --no-video --volume="$volume" \
      --input-ipc-server="$sock" "https://youtu.be/$id" \
      >/tmp/radio-mpv.log 2>&1 &
    disown
    sleep 1
  }

  if ! _radio_mpv_alive; then
  ...
  while true; do
    ...
    input=$(gum input --placeholder "New video (q to quit)")
    ...
    if [[ $input == (q|quit|exit) ]]; then
      ...
      if _radio_mpv_alive; then
        print -r -- '{"command":["quit"]}' | nc -U -w1 "$sock" >/dev/null 2>&1
      fi
      rm -f "$sock"
      break
    fi

    id=$(_radio_extract_id "$input")
    if [[ -z $id ]]; then
    ...
    if ! print -r -- "{\"command\":[\"loadfile\",\"https://youtu.be/$id\",\"replace\"]}" \
      | nc -U -w1 "$sock" >/dev/null 2>&1; then
      gum style --foreground 1 "MPV closed while changing the video. Restarting..."
      _radio_start "$id"
    fi
  done

  unfunction _radio_mpv_alive _radio_start 2>/dev/null
}
alias ra="radio"
