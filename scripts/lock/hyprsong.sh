# Based on hyprlock-music.sh project.
# Dependencies: playerctl (required), curl (for remote art), ImageMagick (for cropping)

# Hyprmusic v1 - first release which is actually a fork.
# ============================================================================
...
generate_progress_bar() {
  ...
  # Treat 97%+ as 100% to handle players switching tracks early
  [[ $percent -ge 97 ]] && percent=100
...
create_square_cover() {
  ...
  if have convert; then
    convert "$input" -auto-orient -gravity center \
      ...
      -quality 95 "$output" && printf '%s' "$output" && return
  fi
...
--title)
  ...
  printf '%s\n' "$(trim_string "${title:-No media source.}" 23)"
  ;;

--artist)
  artist="$(get_metadata 'xesam:artist')"
  printf '%s\n' "$(trim_string "${artist:-}" 25)"
  ;;
